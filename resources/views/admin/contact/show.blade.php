...
@section('content')
    <a href="{{ route('admin.contact.index') }}" class="text-sm text-ssbc-sage hover:text-ssbc-green">{{ __('admin.back') }}</a>
    ...
    <div class="grid lg:grid-cols-3 gap-6">
        <dl class="ssbc-admin-card p-6 space-y-4 lg:col-span-2">
            <div>
                <dt class="ssbc-eyebrow mb-1">{{ __('admin.email_field') }}</dt>
                <dd class="text-sm text-ssbc-dark">{{ $submission->email }}</dd>
            </div>
            <div>
                <dt class="ssbc-eyebrow mb-1">{{ __('admin.phone') }}</dt>
