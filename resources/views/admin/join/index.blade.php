...
@section('title', __('admin.join'))

@section('content')
    <div class="w-12 h-px bg-ssbc-gold mb-4"></div>
    <h1 class="text-2xl font-display font-bold text-ssbc-green mb-8">{{ __('admin.join') }}</h1>
        ...
        <table class="min-w-full text-sm">
            <thead class="bg-ssbc-light text-ssbc-green/80 text-xs uppercase tracking-wider">
                <tr>
                    ...
                    <th class="text-left px-4 py-3">{{ __('admin.organization') }}</th>
                    <th class="text-left px-4 py-3">{{ __('admin.country') }}</th>
            ...
            <tbody class="divide-y divide-ssbc-green/10">
                @forelse($submissions as $sub)
                    <tr>
                        ...
                        <td class="px-4 py-3 text-ssbc-dark/80">{{ $sub->organization }}</td>
                        <td class="px-4 py-3 text-ssbc-dark/70">{{ $sub->country }}</td>
                        ...
                        <td class="px-4 py-3 text-right">
                            <a href="{{ route('admin.join.show', $sub) }}" class="ssbc-link-gold">{{ __('admin.view') }}</a>
                        </td>
                    </tr>
                @empty
                    <tr><td colspan="6" class="px-4 py-8 text-center text-ssbc-sage">{{ __('admin.no_records') }}</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
